...
It appears that while the first measurement is random, the second measurement is the same as the first. This is because after a measurement, the state is no longer in a superposition. Instead, it has collapsed into the state we measured. If we want our superposition, we need to set the whole experiment up again.
